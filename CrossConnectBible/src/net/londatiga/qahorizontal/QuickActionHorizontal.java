 ...
 */
public class QuickActionHorizontal extends CustomPopupWindow {
	 ...
	 */
	public QuickActionHorizontal(View anchor) {
		super(anchor);
	...
	public void show () {
		...
		originalShowXYSize(location, anchor.getHeight(), anchor.getWidth(), anchor.getTop());
	}
	
	/**
	 * Assume the parent is no size so will just show the window at the current location
	 * @param location
	 */
	public void showXY(int[] location) {
	    showXYSize(location, 20, 0, 0 );
	}
	
	/**
	 * Show popup window given specific coordinates and size
	 * @param location
	 */
    public void originalShowXYSize(int[] location, int height, int width, int top) {
        preShow();

        Rect anchorRect     = new Rect(location[0], location[1], location[0] + width, location[1] 
                            + height);
    ...
    public void showXYSize(int[] location, int height, int width, int top) {
        ...
        int boxWidth       = root.getMeasuredWidth();
        int boxHeight      = root.getMeasuredHeight();

        int screenWidth     = windowManager.getDefaultDisplay().getWidth();
        int screenHeight  = windowManager.getDefaultDisplay().getHeight();

        
        //Currently hard coded to be 80 pixels above where it was pressed (when on top)
        int xPos            = (screenWidth - boxWidth) / 2;
        int yPos            = anchorRect.top - 80 ;
//        int yPos            = anchorRect.top -  rootHeight;  //if ontop is false
        
        boolean onTop       = true;

        if (boxHeight > yPos) {
        	//If theres not enough space on the top then make it so that its 160 pixels under where it was pressed
        	//240 because of additional 80 subtracted before
        	//Make it appear on the bottom
        	yPos = yPos + 240;
        	onTop = false;
        }
...
//        }
	...
	private View getActionItem(String title, Drawable icon, OnClickListener listener) {
		LinearLayout container	= (LinearLayout) inflater.inflate(R.layout.action_item_horizontal, null);

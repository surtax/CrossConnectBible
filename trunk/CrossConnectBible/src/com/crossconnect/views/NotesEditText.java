...
public class NotesEditText extends EditText {
	 ...
	 * @param bibleTextView
	 */
	public void initialise(ImageButton lockBtn, final BibleTextViewImpl bibleTextView) {
...
//    }
	...
	//The line spacing between two lines
	private int gap = 0;
	
	//Hacky way to get the gap difference between the lines - add new line find difference and then remove it
	private void getGap() {
		if (gap == 0) {
			append("\n\n");
			gap = getLineBounds(getLineCount()-1, null) - getLineBounds(getLineCount()-2, null);
			this.getText().delete(getText().length()-2,getText().length());
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		//Set the gap if it has not yet been set - should only do it once else screws up scrolling
		getGap();
...
//    	setAutoLinkMask(Linkify.ALL);
    ...
    /**
     * 
     * @deprecated superceded by initialise()
     * @param bibleTextView
     */
    @Deprecated 
	public void setBibleTextView(BibleTextViewImpl bibleTextView) {
		
		final BibleTextViewImpl bibleTV = bibleTextView;
        ...
        lockBtn.setOnClickListener(new OnClickListener(){
            ...
            public void onClick(View v) {
            	...
            	if(isEnabled()) {
            		//If moving from edit to lock mode then save the contents
            		notesService.insertUpdate(bibleTV.getBibleText().getKey(), getText().toString(), getReferences(), getLabels());
            	} 
            	edit();
            	if(isEnabled()) {
            		requestFocus();
